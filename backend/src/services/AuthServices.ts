...
class AuthServices {
  ...
  async recoverPassword(email: string) {
    ...
    const envExpiration = parseInt(
      process.env.EXPIRATION_TOKEN_PASSWORD_RESET ?? ""
    );
    ...
    require("fs").writeFileSync(
      `${__dirname}/../emails/${user.name}-${expToken.toISOString()}.html`,
      emailBody,
      "utf8"
    );
    ...
    require("fs").writeFileSync(
      `${__dirname}/../emails/${user.name}-${expToken.toISOString()}.txt`,
      emailText,
      "utf8"
    );
